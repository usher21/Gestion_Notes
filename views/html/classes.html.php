<main class="w-full h-screen pt-28">
    <header class="flex items-center justify-between px-4 my-3">
        ...
        <button class="px-8 rounded-lg py-3 bg-cyan-950 text-white">Ajouter une nouvelle classe</button>
    </header>

    <?php

    use Model\Managers\ClasseManager;

    $classeManager = new ClasseManager;
    $classeManager->loadClasses();
    ...
    <?php foreach($classes as $classe): ?>
        <ul class="grid grid-cols-4 w-full">
            <li class="text-lg text-cyan-950 py-3 border-r-2 px-3"><?= $classe->getLabel() ?></li>
            <li class="text-lg text-cyan-950 py-3 border-r-2 px-3"><?= $classe->getSize() ?></li>
            <li class="text-lg text-cyan-950 py-3 border-r-2 px-3"><?= $classe->getPlaceNumber() ?></li>
...
<div class="modal-container hidden absolute top-0 w-full h-full flex items-center justify-center">
    ...
    <div class="modal w-1/2 h-2/3 rounded-lg bg-white shadow-lg">
        ...
        <main class="modal-body flex justify-center px-6 mt-8 overflow-scroll">
